...
public class BossLaserCharge1 extends Alien2LaserProjectile {

    private static final int BOSS_LASER_SPEED = 6;

    public BossLaserCharge1(int x, int y) {
        super(x, y);
        initBossLaser();
    }

    private void initBossLaser() {
        File chargeFile = new File("src/images/Corvette_Charge1.png");
        if (chargeFile.exists()) {
            try {
                BufferedImage chargeImage = ImageIO.read(chargeFile);
                int laserWidth = (int)(chargeImage.getWidth() * SCALE_FACTOR * 0.4);
                int laserHeight = (int)(chargeImage.getHeight() * SCALE_FACTOR * 0.6);
